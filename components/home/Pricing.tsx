...
const plans = [
  {
    ...
    colorBorder: 'rgba(52,211,153,0.30)',
    features: [
  ...
  },
  {
    ...
    colorBorder: 'rgba(251,191,36,0.30)',
    features: [
      { text: 'Unlimited API Calls', included: true },
      { text: 'All Tools Access', included: true },
        ...
        <div className="pricing-grid grid grid-cols-1 md:grid-cols-3 gap-5 max-w-[980px] mx-auto items-stretch">
          {plans.map((plan, i) => (
            <div key={i}
              ...
              style={{
                background: plan.isFeatured ? `rgba(124,58,237,0.10)` : 'rgba(120,100,200,0.05)',
                border: `1.5px solid ${plan.isFeatured ? plan.colorBorder : 'rgba(167,139,250,0.14)'}`,
                borderRadius: '8px',
                boxShadow: plan.isFeatured
                  ? `0 0 0 1px rgba(167,139,250,0.20), 0 20px 60px rgba(124,58,237,0.20), 0 0 60px rgba(124,58,237,0.10)`
                  : '0 4px 24px rgba(0,0,0,0.35), 0 0 0 1px rgba(124,58,237,0.06)',
              ...
              onMouseEnter={e => {
                if (!plan.isFeatured) {
                  ...
                  el.style.boxShadow = `0 16px 48px rgba(0,0,0,0.5), 0 0 30px ${plan.colorMuted}, 0 0 0 1px ${plan.colorBorder}`;
                }
              }}
              onMouseLeave={e => {
                if (!plan.isFeatured) {
              ...
              {plan.badge && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 font-jetbrains-mono text-[10px] font-bold tracking-[1.5px] whitespace-nowrap"
                  style={{
                    ...
                    color: plan.color,
                    boxShadow: `0 0 12px ${plan.colorMuted}, 0 4px 16px rgba(0,0,0,0.5)`,
                  }}>
              ...
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 flex items-center justify-center"
                  style={{
                    ...
                    color: plan.color,
                    boxShadow: `0 0 16px ${plan.colorMuted}, inset 0 1px 0 rgba(255,255,255,0.08)`,
                  }}>
                ...
                <div>
                  <span className="font-jetbrains-mono text-[11px] font-bold uppercase tracking-[2px]"
                    style={{ color: 'rgba(224,216,255,0.40)' }}>{plan.name}</span>
                </div>
              </div>

              {/* Price */}
              <div className="mb-2 flex items-start gap-0.5">
                <span className="font-inter text-[18px] font-semibold mt-2" style={{ color: 'rgba(224,216,255,0.50)' }}>$</span>
              ...
              <span className="font-jetbrains-mono text-[11px] mb-7 block"
                style={{ color: 'rgba(224,216,255,0.35)' }}>{plan.period}</span>
              ...
              <ul className="flex-1 flex flex-col gap-2 mb-8">
                {plan.features.map((feat, idx) => (
                  <li key={idx} className="flex items-center gap-3 py-1.5 font-inter text-[13.5px] tracking-[0.1px]"
                    style={{ color: feat.included ? 'rgba(224,216,255,0.80)' : 'rgba(224,216,255,0.25)' }}>
                    {feat.included
                      ? <Check className="w-3.5 h-3.5 shrink-0" style={{ color: 'rgba(52,211,153,0.90)' }} />
                      : <X className="w-3.5 h-3.5 shrink-0" style={{ color: 'rgba(224,216,255,0.20)' }} />
                    }
                    {feat.text}
                  </li>
                ))}
              ...
              <Link href={plan.buttonHref}
                ...
                style={{
                  ...
                  color: plan.isFeatured ? plan.color : 'rgba(224,216,255,0.70)',
                  boxShadow: plan.isFeatured ? `0 0 20px ${plan.colorMuted}, inset 0 1px 0 rgba(255,255,255,0.06)` : 'none',
                }}
                onMouseEnter={e => {
                  const el = e.currentTarget as HTMLElement;
                  ...
                  el.style.boxShadow = `0 8px 24px ${plan.colorMuted}, 0 0 0 1px ${plan.colorBorder}`;
                }}
                onMouseLeave={e => {
                  const el = e.currentTarget as HTMLElement;
                  el.style.background = plan.isFeatured ? plan.colorMuted : 'transparent';
                  el.style.color = plan.isFeatured ? plan.color : 'rgba(224,216,255,0.70)';
                  el.style.transform = '';
                  el.style.boxShadow = plan.isFeatured ? `0 0 20px ${plan.colorMuted}, inset 0 1px 0 rgba(255,255,255,0.06)` : 'none';
                }}>
        ...
        <div className="pricing-vault-btn mt-10 max-w-[980px] mx-auto">
          <Link
            ...
            style={{
              ...
              boxShadow: '0 4px 24px rgba(0,0,0,0.30), 0 0 0 1px rgba(52,211,153,0.08)',
            }}
            onMouseEnter={e => {
              const el = e.currentTarget as HTMLElement;
              el.style.borderColor = 'rgba(52,211,153,0.45)';
              el.style.background = 'rgba(52,211,153,0.08)';
              el.style.transform = 'perspective(800px) translateZ(6px) translateY(-2px)';
              el.style.boxShadow = '0 16px 48px rgba(0,0,0,0.40), 0 0 24px rgba(52,211,153,0.15), 0 0 0 1px rgba(52,211,153,0.28)';
            }}
            onMouseLeave={e => {
              const el = e.currentTarget as HTMLElement;
              ...
              el.style.transform = '';
              el.style.boxShadow = '0 4px 24px rgba(0,0,0,0.30), 0 0 0 1px rgba(52,211,153,0.08)';
            }}
          >
            {/* Corner marks */}
            <span className="absolute top-2 left-2 w-4 h-4 pointer-events-none" style={{ borderTop: '1.5px solid rgba(52,211,153,0.35)', borderLeft: '1.5px solid rgba(52,211,153,0.35)' }} />
            <span className="absolute top-2 right-2 w-4 h-4 pointer-events-none" style={{ borderTop: '1.5px solid rgba(52,211,153,0.35)', borderRight: '1.5px solid rgba(52,211,153,0.35)' }} />
            <span className="absolute bottom-2 left-2 w-4 h-4 pointer-events-none" style={{ borderBottom: '1.5px solid rgba(52,211,153,0.35)', borderLeft: '1.5px solid rgba(52,211,153,0.35)' }} />
            <span className="absolute bottom-2 right-2 w-4 h-4 pointer-events-none" style={{ borderBottom: '1.5px solid rgba(52,211,153,0.35)', borderRight: '1.5px solid rgba(52,211,153,0.35)' }} />
            ...
            <div className="flex items-center gap-5">
              <div className="w-11 h-11 shrink-0 flex items-center justify-center"
                style={{
                  ...
                  boxShadow: '0 0 16px rgba(52,211,153,0.12), inset 0 1px 0 rgba(255,255,255,0.06)',
                }}>
                <Vault className="w-5 h-5" />
              </div>
              <div>
                <p className="font-outfit font-bold text-[15px] tracking-[0.3px] mb-0.5" style={{ color: 'rgba(224,216,255,0.92)' }}>
                  Free Cheats Vault
                </p>
                <p className="font-jetbrains-mono text-[11px]" style={{ color: 'rgba(52,211,153,0.65)' }}>
            ...
            {/* Right — badge + arrow */}
              ...
              <ArrowRight className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1" style={{ color: 'rgba(52,211,153,0.80)' }} />
            </div>
          </Link>
        </div>
      </div>
